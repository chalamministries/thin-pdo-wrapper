 ...
 *
 * Special Thanks to Michael Henretty
 *
 ...
 * @link       https://github.com/chalamministries/thin-pdo-wrapper
 */
 ...
 *
 * @package PDO Wrapper
 * @author Steven Gauerke - 03/20/2023
 * @version 2.0
 */
class PDOWrapper
{
     ...
     * @var string
     */
    public static $LOG_FILE = ini_get('error_log');
    ...
    public function configMaster($host, $name, $user, $password, $port=null, $driver='mysql')
    {
        if (!$this->validateDriver($driver)) {
            throw new Exception('DATABASE WRAPPER::Validate Driver::error, the database you wish to connect to is not supported by your install of PHP.');
        }
    ...
    protected function createConnection($driver, $host, $name, $user, $password, $port=null)
    {
        ...
        // handle any exceptions by catching them and returning false
            ...
            if (self::$LOG_ERRORS == true) {
                error_log('DATABASE WRAPPER::'.print_r($e, true), 3, self::$LOG_FILE);
            }
     ...
     */
    public function select($table, $params = array(), $limit = null, $start = null, $order_by=null, $use_master = false)
    {
        ...
        try {
            ...
            if (self::$LOG_ERRORS == true) {
                error_log('DATABASE WRAPPER::'.print_r($e, true), 3, self::$LOG_FILE);
                error_log('PROBLEM QUERY::' . $sql_str, 3, self::$LOG_FILE);
            }
            $this->pdo_exception = $e;
            return false;
        } catch (Exception $e) {
...
// 
//         foreach ($params as $key=>$val) {
//             $binder = str_replace(':'.$key, "'" . $val . "'", $binder);
//         }
//         
//         if($table == "prospects") {
//             error_log('BIND QUERY::' . $binder, 3, self::$LOG_FILE);
//         }
    ...
    /**
     * method execute.
     * 	- executes a query that modifies the database using the master connection
     *
    ...
    public function queryFetchAllCloseCursor($query, $params=array(), $use_master=false)
    {
        try {
            // decide which database we are selecting from
            $pdo_connection = $use_master ? $this->getMaster() : $this->getSlave();
